...
const colorArray = ["#2185C5", "#7ECEFD", "#FF7F66"];
const circleArray = [];
...
class Circle {
  ...
  update(circleArray) {
    ...
    for (let i = 0; i < circleArray.length; i++) {
      ...
      if (this.radius + circleArray[i].radius > distance) {
    ...
    // mouse
    if (
      getDistance(mouse.x, mouse.y, this.x, this.y) < 100 &&
      this.opacity < 0.2
    ) {
      this.opacity += 0.02;
  ...
  for (let i = 0; i < 300; i++) {
    ...
    if (i !== 0) {
      for (let j = 0; j < circleArray.length; j++) {
        const distance = getDistance(x, y, circleArray[j].x, circleArray[j].y);
        if (radius + circleArray[j].radius > distance) {
